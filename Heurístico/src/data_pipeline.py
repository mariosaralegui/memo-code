"""
Preprocesamiento de Datos y Safety Stock Estocástico (MAD añadido)
"""
import pandas as pd
import numpy as np
...
def calcular_mad(x: pd.Series) -> float:
    """Calcula la Desviación Absoluta de la Mediana (MAD) ignorando NaNs."""
    if len(x) == 0:
        return 0.0
    mediana = x.median()
    return (x - mediana).abs().median()

def procesar_demanda_estocastica(recogidas_path: str, inventario_path: str, z_score: float = 1.28) -> pd.DataFrame:
    '''
    Args:
        recogidas_path (str): Ruta al archivo de datos de recogidas.
        inventario_path (str): Ruta al archivo de datos de inventario.
        z_score (float): Valor z para el cálculo del safety stock. Default es 1.28 para un nivel de seguridad del 90%.
    Returns:
        pd.DataFrame: DataFrame con los datos de demanda procesados.
    '''
    print("[Pipeline] Iniciando análisis de intervalos temporales y Safety Stock...")
    ...
    umbral_seguro_kg = capacidad_practica * 0.80

    # 5. Agregación estadística robusta por contenedor (Uso de MAD)
    ...
    # 6. Cálculo Estocástico (Demanda Pesimista Robusta)
    ...
    print(f"[Pipeline] Numero de contenedores por tramo de frecuencia óptima: {df_final['Opt_Frecuencia_Semanal'].value_counts().to_dict()}")
    return df_final